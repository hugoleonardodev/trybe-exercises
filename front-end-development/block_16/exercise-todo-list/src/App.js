...
function App({ inputTask, addTask, removeTask, selectTask, listTodo, selected, position, input, textTodo }) {
// class App extends Component {
  ...
  //   const { listTodo, selected } = this.state;
    return (
      <div className="App">
        <InputTodo inputTask={inputTask} addTask={addTask} listTodo={listTodo} />
              ...
              listTodo.map((todo, index) => (
                <li key={index + 1} >
                  <Item
                    content={todo}
                    value={index}
                    selectTask={(event) => selectTask(event)}
                  />
                </li>
              ))
            }
...
const mapStateToProps = (state) => ({
  ...
  selected: state.appReducer.selected,
  position: state.appReducer.position,
